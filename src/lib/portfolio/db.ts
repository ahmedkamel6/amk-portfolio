...
const _getProjects = async (): Promise<ProjectDetail[]> => {
  ...
  return rows.map((r) => ({
    ...
    client: r.client,
    videoUrl: r.videoUrl,
    coverImage: r.coverImage?.startsWith('data:image') ? `/api/project-image/${r.slug}/cover` : r.coverImage,
    driveUrl: r.driveUrl,
    thumbnailUrl: r.thumbnailUrl?.startsWith('data:image') ? `/api/project-image/${r.slug}/thumbnail` : r.thumbnailUrl,
