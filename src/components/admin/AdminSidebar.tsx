...
import { useContentStore } from '@/lib/portfolio/content-store'
...
export function AdminSidebar() {
  ...
  const mode = useContentStore((s) => s.theme.mode)
  const toggleTheme = useContentStore((s) => s.toggleTheme)
    ...
    <>
      ...
      <div className="border-t border-[var(--border)] p-3">
        ...
        >
          ...
          <span className="font-medium">Switch to {mode === 'dark' ? 'Light' : 'Dark'}</span>
        </button>
        <Link
          href="/"
          className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm text-[var(--text-secondary)] transition-colors hover:bg-[var(--surface)] hover:text-[var(--text-primary)]"
