    ...
    <div ref={ref} className="relative grid grid-cols-1 gap-8 md:grid-cols-2 md:gap-16">
      <div className="absolute left-4 top-2 z-20 md:left-1/2 md:-translate-x-1/2">
        ...
        >
          ...
          <motion.span className="absolute inset-0 rounded-full border border-emerald-glow/40" animate={{ scale: [1, 1.6], opacity: [0.6, 0] }} transition={{ duration: 2, repeat: Infinity, delay: index * 0.3 }} />
        </motion.div>
      </div>
      <motion.div
      ...
      >
        <div className="group relative overflow-hidden rounded-3xl border border-[var(--border)] bg-gradient-to-b from-[#0d0f14] to-[#3a4559] p-7 backdrop-blur-xl transition-all duration-500 hover:border-emerald-glow/30 hover:bg-emerald-glow/[0.03]">
  ...
  return (
    <section id="process" className="relative w-full overflow-hidden pt-28 pb-32 md:pt-32 md:pb-48" ref={containerRef}>
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute left-1/2 top-0 h-[80vh] w-[40vw] -translate-x-1/2 rounded-full opacity-25 blur-[120px]" style={{ background: 'radial-gradient(circle, color-mix(in srgb, var(--emerald-glow) 20%, transparent), transparent 70%)' }} />
      </div>
